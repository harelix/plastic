...
class Selector extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = { current: "Simple" , remote : null};
  }
  ...
  onLabelClick = label => {
    return event => {
      ...
      if(typeof samples[label]=='function'){
        let remoteFunc = samples[label]();
        let result = {};
        remoteFunc
        .then((response) => response.json())
        .then((responseJson) => {
          setImmediate(() => this.props.onSelected(responseJson));
        })
        .catch((error) => {
          console.error(error);
        });
  ...
  componentWillReceiveProps(nextProps){
    ...
    if(!this.state){
      //var remotePromise = this.state.remote();
    }
...
class App extends Component {
  constructor(props) {
    ...
    const { schema, uiSchema, formData, validate } = {};
    this.state = {
      form: false,
      schema,
      uiSchema,
  ...
  componentDidMount() {
    ...
    if (hash && typeof hash[1] === "string" && hash[1].length > 0) {
      ...
      this.load(samples.remoteElastic);
    }
  }
  ...
  onDocumentSelected = (index, type, document) => {
    let url = 'http://localhost:8080/api/elastic/document/{0}/{1}/{2}'.format(index, type, document._id)
    this.setState({index, type , selectedDocumentId : document._id})
    fetch(url)
      .then((response) => response.json())
      .then((responseJson) => {
        setImmediate(() => this.load(responseJson));
      })
      .catch((error) => {
        console.error(error);
    });
  ...
  onFormDataChange = ({ formData }) =>
    ...
    onFormSubmit = (args) => {
      this.state.uuid = this.state.selectedDocumentId;
      let {index , type} = this.state;
      delete this.state.index;
      delete this.state.type;
      fetch('http://localhost:8080/api/elastic/document/'+ index +'/' + type, {
        method: 'POST',
        headers: {
